...
critical_words = ['CV', 'data', 'science']
...
def lambda_handler(event, context):
    ...
    #generate random number for response id
    rid = np.random.randint(1000000)
    
    #instantiate dynamodb service
    dynamodb = boto3.resource('dynamodb')
    
    #create instance of dynamodb table
    table = dynamodb.Table('my-portfolio-data-table')


    
    #insert items from the website into the dynamodb table
    ...
    # --- Insert your code here ---
    sentiment = json.loads(json.dumps(comprehend.detect_sentiment(Text=dec_dict['message'], LanguageCode='en'), sort_keys=True, indent=4)) # <---Insert code to get the sentiment with AWS comprehend
    # -----------------------------
    
    # --- Insert your code here ---
    key_phrases = json.loads(json.dumps(comprehend.detect_key_phrases(Text=dec_dict['message'], LanguageCode='en'), sort_keys=True, indent=4)) # <--- Insert code to get the key phrases with AWS comprehend
    # -----------------------------
    ...
    email_text =  email_response(dec_dict['name'], critical_words, key_phrases, sentiment)
